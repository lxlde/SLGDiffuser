...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        if conditioning is not None:# 执行这里
    ...
    def ddim_sampling(self, cond, shape,
        ...
        return img, intermediates    #原来返回的是img,intermediates
    ...
    @torch.no_grad()
    def get_Pred_x0(self,index,z,device,e_t):
        b, *_, device = *z.shape, device
        ...
        a_t = torch.full((b, 1, 1, 1), alphas[index], device=device)
        sqrt_one_minus_at = torch.full((b, 1, 1, 1), sqrt_one_minus_alphas[index],device=device)
        pred_x0 = (z - sqrt_one_minus_at * e_t) / a_t.sqrt()
        return pred_x0
